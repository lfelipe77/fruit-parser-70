...
-- RAFFLES (new structure)
create table if not exists public.raffles_new (
...
-- Copy data from existing raffles if it exists
do $$
begin
  if exists (select 1 from information_schema.tables where table_name = 'raffles' and table_schema = 'public') then
    insert into public.raffles_new (
      id, created_at, updated_at, owner_user_id, title, description, image_url, 
      total_tickets, ticket_price, status
    )
    select 
      id, created_at, updated_at, user_id, title, description, image_url,
      total_tickets, ticket_price, 'draft'::raffle_status
    from public.raffles
    on conflict (id) do nothing;
  end if;
end $$;

-- Replace old raffles table
drop table if exists public.raffles cascade;
alter table public.raffles_new rename to raffles;
...
create table if not exists public.transactions_new (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
...
-- Copy data from existing transactions if it exists
do $$
begin
  if exists (select 1 from information_schema.tables where table_name = 'transactions' and table_schema = 'public') then
    insert into public.transactions_new (
      id, created_at, user_id, amount, provider_payment_id, status
    )
    select 
      id, created_at, user_id, amount, 'asaas'::payment_provider, 
      coalesce(payment_id, id::text), 
      case 
    ...
    where exists (select 1 from public.raffles r where r.id = t.raffle_id)
    on conflict (id) do nothing;
  end if;
end $$;
...
create unique index if not exists ux_tx_provider_pid on public.transactions(provider, provider_payment_id);
...
-- Copy data from existing tickets if it exists
do $$
begin
  if exists (select 1 from information_schema.tables where table_name = 'tickets' and table_schema = 'public') then
    insert into public.tickets_new (
      id, created_at, raffle_id, user_id, number, status
    )
    select 
      id, created_at, raffle_id, user_id, 
      coalesce(ticket_number, row_number() over (partition by raffle_id order by created_at)),
      case 
        when is_paid = true then 'paid'::ticket_status
        else 'reserved'::ticket_status
      end
...
-- STORAGE: bucket + policies (idempotente e tolerante a permissão)
do $$
begin
...
exception when others then
  -- Se faltar permissão, ignore (crie o bucket via UI).
  null;
end $$;

-- Recria policies sem IF NOT EXISTS
