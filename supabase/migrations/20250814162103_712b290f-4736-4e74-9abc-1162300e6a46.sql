-- Complete Raffle System Schema (Corrected for existing structure)
...
-- Update existing raffles table structure
...
ADD COLUMN IF NOT EXISTS winner_user_id uuid references public.user_profiles(id) on delete set null,
ADD COLUMN IF NOT EXISTS draw_timestamp timestamptz;

-- Update status column to use enum (if it's not already)
DO $$
BEGIN
    ALTER TABLE public.raffles ALTER COLUMN status TYPE raffle_status USING status::raffle_status;
EXCEPTION
    WHEN OTHERS THEN
...
-- Update ticket_price and total_tickets to match schema
ALTER TABLE public.raffles 
...
DO $$
BEGIN
    ALTER TABLE public.raffles ADD CONSTRAINT raffles_total_tickets_check CHECK (total_tickets > 0);
EXCEPTION
    WHEN duplicate_object THEN NULL;
END$$;
...
DO $$
BEGIN
    ALTER TABLE public.raffles ADD CONSTRAINT raffles_ticket_price_check CHECK (ticket_price >= 0);
EXCEPTION
    WHEN duplicate_object THEN NULL;
END$$;
...
DO $$
BEGIN
    ALTER TABLE public.raffles ADD CONSTRAINT raffles_slug_key UNIQUE (slug);
EXCEPTION
    WHEN duplicate_object THEN NULL;
END$$;
...
-- Create indexes
create index if not exists idx_raffles_owner on public.raffles(user_id);
...
-- Update RLS policies for raffles
...
on public.raffles for select to public using (
  status::text in ('approved','closed','delivered')
  or user_id = auth.uid()
...
using (
  user_id = auth.uid()
  and status::text in ('draft','under_review')
)
...
where r.status::text in ('approved','closed','delivered');
...
begin
  ...
  insert into public.raffles(
    user_id, title, description, category_id, image_url,
    prize_value, total_tickets, ticket_price, draw_date, status
  ) values (
    auth.uid(), p_title, p_description, p_category_id, p_image_url,
    p_prize_value, p_total_tickets, p_ticket_price, p_draw_date, 'under_review'::raffle_status
  )
...
begin
  ...
  set winner_user_id = v_winner_user, draw_timestamp = now(), status = 'closed'::raffle_status
