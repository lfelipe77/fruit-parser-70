...
SELECT
  ...
  NULL::text as color_class,  -- synthetic for now
...
-- Add subcategory_id column to raffles table if it doesn't exist
...
CREATE POLICY IF NOT EXISTS "public_read_subcategories"  
...
((SELECT id FROM public.categories WHERE slug = 'eletronicos' LIMIT 1), 'Smartphones', 'smartphones', 1),
((SELECT id FROM public.categories WHERE slug = 'eletronicos' LIMIT 1), 'Tablets', 'tablets', 2),
((SELECT id FROM public.categories WHERE slug = 'eletronicos' LIMIT 1), 'Laptops', 'laptops', 3)
