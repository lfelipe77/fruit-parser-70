-- Step 0: Make finalize_paid_purchase idempotent with advisory lock and early exits
-- Recreate function with same signature, keeping provider as 'asaas' for provider_payment_id checks
...
BEGIN
  ...
  -- Optionally upsert reservation audit cache
  PERFORM public.upsert_reservation_audit(p_reservation_id) ON CONFLICT DO NOTHING;
