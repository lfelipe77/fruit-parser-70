-- Fix my_tickets_ext_v6 to only include paid transactions with corrected grouping
...
SELECT 
  ...
  -- Use COUNT(*) for accurate ticket counting
  ...
  -- Calculate amount raised per raffle (using window function to avoid grouping issues)
  SUM(SUM(tr.amount)) OVER (PARTITION BY t.raffle_id) as amount_raised,
  -- Progress percentage calculation
  CASE 
    WHEN COALESCE(r.goal_amount, 0) > 0 
    THEN LEAST(100, (SUM(SUM(tr.amount)) OVER (PARTITION BY t.raffle_id) * 100.0 / r.goal_amount)::integer)
    ELSE 0 
