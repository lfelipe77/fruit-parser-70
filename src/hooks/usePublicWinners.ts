...
export function usePublicWinners(limit = 50) {
  return useQuery({
    ...
    queryFn: async (): Promise<PublicWinner[]> => {
      console.log('[usePublicWinners] Starting query...');
      ...
      try {
        ...
        if (error) {
          console.warn('[usePublicWinners] RLS blocked raffle_winners, error:', error.message);
          throw error; // Force fallback
        }
        
        console.log('[usePublicWinners] Successfully got', rawData?.length || 0, 'winners from raffle_winners');
        ...
        console.warn('[usePublicWinners] Falling back to v_public_winners view');
        ...
        if (viewError) {
          console.error('[usePublicWinners] View query failed:', viewError.message);
          throw viewError;
        }
        
        console.log('[usePublicWinners] Fallback got', viewData?.length || 0, 'winners from v_public_winners');
